...
/*
|--------------------------------------------------------------------------
| Numeric Wastage In modal — ported from the former NumericWastageInView.
| Still calls stockApi.postNumericWasteIn -> POST /stock/numeric-waste-in,
| a route that isn't registered in schainbackend/routes/api.php even
| though the controller/service method exists (backend gap, reported not
| fixed — see the Stock Management consolidation plan).
|--------------------------------------------------------------------------
*/
...
<template>
  ...
  >
    ...
    <p class="text-xs text-slate-500">
      Amount (no. of pcs × amount per pc) moves cash from "Given by" to "Given to" automatically.
    </p>
