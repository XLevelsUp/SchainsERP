...
import type { Item, StockOutFormValues, StockOutItemInput, UserDetailListItem } from '@/types'

/*
|--------------------------------------------------------------------------
| Stock Out modal — ported from the former StockOutView.vue
|--------------------------------------------------------------------------
| Validation/payload/calculations are unchanged from the standalone page;
| only the shell (page -> modal) and data source (props vs. onMounted
| fetch) changed.
|--------------------------------------------------------------------------
*/
...
function makeEmptyItem(): StockOutItemInput {
  return { item_id: null, grams: null, touch: 100, waste_total: 0, remarks: '', item_remarks: '' }
}
...
<template>
  <BaseModal title="Stock Out" badge="OUT" badge-class="bg-red-600" max-width="max-w-4xl" @close="emit('close')">
    <p
      v-if="formError"
    ...
    <form class="flex flex-col gap-6" @submit.prevent="handleSubmit">
      ...
      <div class="flex items-center justify-end gap-3 border-t border-slate-200 pt-4">
        ...
        <BaseButton type="submit" :disabled="isSaving">{{ isSaving ? 'Recording…' : 'Record stock out' }}</BaseButton>
      </div>
    </form>
  </BaseModal>
</template>
