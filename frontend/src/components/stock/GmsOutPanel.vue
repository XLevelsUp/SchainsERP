...
/*
|--------------------------------------------------------------------------
| GMS Out — inline multi-row entry, not a modal
|--------------------------------------------------------------------------
| Replaces GmsOutModal.vue. Same shared-Submit orchestration contract as
| StockOutPanel.vue (see its comment) — stacks below StockOutPanel in the
| OUT column since it hits a different endpoint (POST /stock/gms-out)
| with a different field set (stone/thread/hall_mark/mtouch), so it can't
| just reuse Stock Out's row shape; it's kept as its own panel/table
| rather than crammed into fields it doesn't match.
|
| given_by is optional here (defaults server-side to the acting user),
| unlike GMS In — matching GmsOutModal's original comment. This panel
| still always sends headId as given_by for consistency with Stock Out's
| page-context model (no per-row picker either way).
|
| Metal picker (see StockOutPanel's comment for the full contract) is
| scoped to headId, same direction as Stock Out.
|--------------------------------------------------------------------------
*/
...
const rows = ref<GmsOutItemInput[]>([])
...
function addRow() {
  ...
  if (props.givenTo === null) {
    toast.show('Select a user (left panel) before adding a GMS Out row.', 'error')
    return
  }
...
<template>
  <BaseCard v-if="rows.length > 0" :padded="false">
    <div class="flex items-center justify-between border-b border-slate-200 px-3 py-2">
      <h2 class="text-sm font-semibold text-slate-900">GMS Out</h2>
