...
/*
|--------------------------------------------------------------------------
| GMS In modal — mirrors GmsInRequest.php
|--------------------------------------------------------------------------
| Replaces GmsInPanel.vue: GMS In is a modal action here, not part of the
| inline add-row/shared-Submit grid — same reasoning and pattern as
| ItemChangeModal/ItemConversionModal/GmsOutModal (see
| StockManagementView.vue's comment). Requested to match the legacy
| "Create new Gms Entry" dialog layout (Item/Grams/Stone/Wastage/Hallmark/
| MTouch/MTouchWastage/Total table, Add + Close/Save).
|
| Scoped to givenBy/headId props (from the page's UserPickerPanel +
| logged-in head), same as the panel it replaces and the same pattern as
| GmsOutModal — NOT an independent user picker inside the modal like the
| legacy screenshot shows. The page already has one source of truth for
| "who" (UserPickerPanel, left column) that scopes every panel/modal
| here; a second picker inside this one would just create two ways to
| pick a user that could disagree. given_by = givenBy (selected user),
| given_to = headId — same direction as the panel (both required per
| GmsInRequest, unlike GMS Out where given_by is optional).
|
| Legacy screenshot also shows a "Customer Touch" dropdown — not part of
| GmsInRequest's validated payload (checked: no such field), and
| customer-touch is actually a global item master list
| ({item_id, item_name, is_active}), not a per-customer preference.
| Left out here since its exact legacy behavior (item-list filter? pure
| decoration?) isn't known — flag if it should filter the Item dropdown.
|
| Metal picker (see GmsOutModal's comment) preserved exactly, scoped to
| givenBy (the source user), same direction as Stock In.
|--------------------------------------------------------------------------
*/
...
const rows = ref<GmsInItemInput[]>([makeEmptyRow()])
...
async function handleSubmit() {
  ...
  if (props.givenBy === null) {
    toast.show('Select a user (left panel) before recording a GMS In.', 'error')
    return
  }
  if (!validate()) return
...
<template>
  <BaseModal title="GMS In" badge="IN" badge-class="bg-emerald-600" max-width="max-w-5xl" @close="emit('close')">
    <p v-if="givenBy === null" class="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
      Select a user in the left panel before recording a GMS In.
    </p>
    ...
    <form class="flex flex-col gap-6" @submit.prevent="handleSubmit">
      ...
      <div class="flex items-center justify-end gap-3 border-t border-slate-200 pt-4">
        ...
        <BaseButton type="submit" :disabled="isSaving">{{ isSaving ? 'Recording…' : 'Record GMS in' }}</BaseButton>
      </div>
    </form>
