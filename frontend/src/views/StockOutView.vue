...
import { Plus, RefreshCw, Trash, PackageMinus } from 'lucide-vue-next'
...
function makeEmptyItem(): StockOutItemInput {
  return {
    item_id: null,
    grams: null,
    touch: null,
...
function makeEmptyForm(): StockOutFormValues {
  return {
    given_by: null,
    given_to: null,
    retailer_id: null,
...
const form = reactive<StockOutFormValues>(makeEmptyForm())
const formError = ref('')
const isSaving = ref(false)
const fieldErrors = reactive<Record<string, string>>({})
const lastResult = ref<StockOutResult | null>(null)
...
// Mirrors StockInventoryService::createStockOut()'s fallback formula
// exactly — display-only preview; the actual values are computed
// server-side so they can never drift from what gets stored.
...
const resultColumns: DataTableColumn<StockOutResult['stocks'][number]>[] = [
...
/*
|--------------------------------------------------------------------------
| Validation — mirrors StockOutRequest exactly
|--------------------------------------------------------------------------
*/
...
async function handleSubmit() {
  if (!validate()) return
  if (!auth.user) {
    formError.value = 'You must be signed in to record a stock out.'
    return
  }
  ...
  formError.value = ''
  try {
    const result = await stockApi.postStockOut({ ...form, items: form.items.map((i) => ({ ...i })) }, auth.user.user_id)
    lastResult.value = result
    toastStore.show(`Stock out recorded — bill #${result.bill_id}.`, 'success')
    ...
    if (err instanceof ApiError) {
      ...
      formError.value = 'Failed to record stock out.'
      toastStore.show('Failed to record stock out.', 'error')
    }
  } finally {
...
<template>
  <div>
    <PageHeader
      title="Stock Out"
      description="Move item stock from one user to another (New Out flow)."
    >
      <template #actions>
    ...
    <BaseCard v-else class="mb-6">
      ...
      <form class="flex flex-col gap-6" @submit.prevent="handleSubmit">
        <section class="grid gap-3 sm:grid-cols-3">
          ...
          <BaseSelect
            id="retailer_id"
            :model-value="form.retailer_id"
            label="Retailer (optional)"
            size="sm"
            placeholder="None"
            :options="userOptions"
            @update:model-value="(v) => (form.retailer_id = v as number | null)"
          />
          <BaseInput
            id="added_at"
        ...
        <div class="flex items-center gap-3 border-t border-slate-200 pt-4">
          <BaseButton type="submit" :icon="PackageMinus" :disabled="isSaving">
            {{ isSaving ? 'Recording…' : 'Record stock out' }}
          </BaseButton>
        </div>
      </form>
    </BaseCard>
    ...
    <BaseCard v-if="lastResult">
      <h2 class="mb-1 text-sm font-semibold text-slate-900">
        Last recorded batch — Bill #{{ lastResult.bill_id }}
      </h2>
      <p class="mb-4 text-xs text-slate-500">
        {{ lastResult.stocks[0] ? userName(lastResult.stocks[0].given_by) : '' }} — recorded
        {{ formatDateTime(lastResult.stocks[0]?.added_at) }}
      </p>
      <DataTable :columns="resultColumns" :rows="lastResult.stocks">
        ...
        <template #waste_total="{ value }">{{ Number(value).toFixed(3) }}</template>
